############################################################
# ALFF / fALFF (instead of FC)
# - Same modeling framework as your FC script
# - Keeps GDSS in model
# - Keeps ChaCo borrowing + stroke-only ChaCo moderation
# - Works with long-format merged CSV:
#     study_id, event_name, feature_type, feature_name, value,
#     behav, gdss_score, age, sex, group, (optional) time_mo
#
# EXPECTED feature_types from your MATLAB export:
#   "ALFF_NET_MEAN", "FALFF_NET_MEAN", and optionally "ALFF_ROI", "FALFF_ROI"
#   plus the CHACO_* feature types if you include them in merged CSV.
############################################################
...
# ---- choose which ALFF/fALFF feature types to analyze ----
# Network means are small + interpretable; ROI-level is big (86*subjects*visits)
feature_types_keep <- c(
  "ALFF_NET_MEAN","FALFF_NET_MEAN",
  # uncomment if you exported ROI-level values
  "ALFF_ROI","FALFF_ROI"
  # keep ChaCo if present / you want it in this run
...
add_model_family <- function(feature_type) {
  ft <- toupper(trimws(as.character(feature_type)))
  dplyr::case_when(
    ft == "ALFF_NET_MEAN"  ~ "ALFF_NETWORK",
    ft == "FALFF_NET_MEAN" ~ "FALFF_NETWORK",
    ft == "ALFF_ROI"       ~ "ALFF_ROI",
    ft == "FALFF_ROI"      ~ "FALFF_ROI",
...
cat("\nUnique feature_type values in your CSV (truth):\n")
...
############################################################
# Borrow behaviour + GDSS across timepoints ONLY for CHACO rows
# (same as your FC script, kept here)
############################################################
...
# - Moderates ALFF/fALFF features by subject-level ChaCo summary
############################################################
chaco_pick_feature_type <- c("CHACO_NET_MEAN", "CHACO_NET_WITHIN_MEAN", "CHACO_NET_BETWEEN_MEAN")
chaco_pick_feature_name <- NULL

# IMPORTANT CHANGE vs FC version:
# moderation_families now includes ALFF/fALFF families (NOT FC)
moderation_families <- c("ALFF_NETWORK","FALFF_NETWORK","ALFF_ROI", "FALFF_ROI")
...
chaco_subject <- dat_model %>%
  ...
  filter(!is.na(value)) %>%
  {
    if (!is.null(chaco_pick_feature_name)) dplyr::filter(., feature_name == chaco_pick_feature_name) else .
  } %>%
...
write_csv(results_chaco_mod,
          file.path(out_dir, "STROKEONLY_ChacoModeration_ALFF_fALFF_models_mixed.csv"))
...
write_csv(mod_terms,
          file.path(out_dir, "STROKEONLY_ChacoModeration_vitality_by_chaco_with_FDR_ALFF_fALFF.csv"))
...
## value ~ behav * group + gdss_score + covariates + (1|study_id)
############################################################
...
############################################################
## SECOND RUN: Residualized vitality model
...
write_csv(results, file.path(out_dir, "ALL_ALFF_fALFF_all_feature_models_mixed.csv"))
write_csv(results_resid, file.path(out_dir, "ALL_ALFF_fALFF_all_feature_models_mixed_BEHAV_RESID.csv"))

############################################################
## FDR within family + effect_type
############################################################
terms_fdr <- results_tagged %>%
  filter(effect_type != "other", !is.na(p.value)) %>%
  group_by(model_family, effect_type) %>%
  mutate(p_fdr = p.adjust(p.value, method = "BH")) %>%
  ungroup()

terms_resid_fdr <- results_resid_tagged %>%
  filter(effect_type != "other", !is.na(p.value)) %>%
  group_by(model_family, effect_type) %>%
  mutate(p_fdr = p.adjust(p.value, method = "BH")) %>%
  ungroup()

write_csv(terms_fdr, file.path(out_dir, "ALL_ALFF_fALFF_terms_with_FDR_mixed.csv"))
write_csv(terms_resid_fdr, file.path(out_dir, "ALL_ALFF_fALFF_terms_with_FDR_mixed_BEHAV_RESID.csv"))

sig_vit_main <- terms_fdr %>% filter(effect_type == "vitality_main", p_fdr < alpha_fdr)
sig_vit_int  <- terms_fdr %>% filter(effect_type == "vitality_by_group", p_fdr < alpha_fdr)
sig_gdss     <- terms_fdr %>% filter(effect_type == "gdss_main", p_fdr < alpha_fdr)

sig_resid_main <- terms_resid_fdr %>% filter(effect_type == "vitality_resid_main", p_fdr < alpha_fdr)
sig_resid_int  <- terms_resid_fdr %>% filter(effect_type == "vitality_resid_by_group", p_fdr < alpha_fdr)

write_csv(sig_vit_main,   file.path(out_dir, "ALL_sig_vitality_main_FDR_ALFF_fALFF.csv"))
write_csv(sig_vit_int,    file.path(out_dir, "ALL_sig_vitality_by_group_FDR_ALFF_fALFF.csv"))
write_csv(sig_gdss,       file.path(out_dir, "ALL_sig_gdss_main_FDR_ALFF_fALFF.csv"))
write_csv(sig_resid_main, file.path(out_dir, "ALL_sig_vitality_resid_main_FDR_ALFF_fALFF.csv"))
write_csv(sig_resid_int,  file.path(out_dir, "ALL_sig_vitality_resid_by_group_FDR_ALFF_fALFF.csv"))

# Save per-family files (optional but handy)
...
for (fam in families_present) {
  ...
  res_fdr <- terms_fdr %>% filter(model_family == fam)
...
cat("\n✅ Done (ALFF/fALFF with GDSS). Outputs saved to:\n", out_dir, "\n")